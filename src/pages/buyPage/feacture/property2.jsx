...
// 1. Accept 'properties' as a prop
const PropertyListingPage = ({ properties = [] }) => {
  ...
  // --- Filtering Logic ---
  // 2. Filter the incoming 'properties' prop based on sidebar state
  const filteredProperties = useMemo(() => {
    return properties.filter(property => {
      
      // Filter by Beds (Greater than or equal to selected)
      if (filters.bedrooms && property.beds < filters.bedrooms) return false;

      // Filter by Baths (Greater than or equal to selected)
      if (filters.bathrooms && property.baths < filters.bathrooms) return false;
      ...
      // Filter by Features
      if (filters.features.length > 0) {
        // If data doesn't have features array, exclude it
  ...
  return (
    <div className="w-full p-4 md:p-4 font-sans text-gray-800 ">
      <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-1/4 min-w-[280px]">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 sticky top-4">
            ...
            {/* Header */}
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-poppin font-bold text-[#2B8CAD]">Filters</h2>
              <button 
                onClick={resetFilters}
                className="flex items-center gap-1 text-xs font-poppin text-[#2B8CAD] hover:text-[#237896] transition-colors"
              >
                <RotateCcw size={12} />
                Reset
              </button>
            ...
            {/* Price Range */}
            <div className="mb-8">
              <label className="text-sm font-poppin font-bold text-[#2B8CAD] mb-4 block">Price Range</label>
              <div className="relative h-2 bg-gray-200 rounded-full mb-3 mt-2">
                <div className="absolute left-0 right-1/4 h-full bg-[#2B8CAD] rounded-full"></div>
                <div className="absolute left-0 top-1/2 transform -translate-y-1/2 w-4 h-4 bg-white border-2 border-[#2B8CAD] rounded-full cursor-pointer shadow-sm"></div>
                <div className="absolute right-1/4 top-1/2 transform -translate-y-1/2 w-4 h-4 bg-white border-2 border-[#2B8CAD] rounded-full cursor-pointer shadow-sm"></div>
              </div>
              <div className="flex justify-between  text-xs font-poppin font-bold text-gray-600">
                <span>$0</span>
                <span>$6M+</span>
              </div>
            </div>
            ...
            {/* Bedrooms */}
            <div className="mb-6">
              <label className="text-sm font-poppin font-bold text-[#2B8CAD] mb-3 block">Bedrooms</label>
              <div className="grid grid-cols-3 gap-2 mb-2">
                {[1, 2, 3].map(num => (
                  <button 
                    key={num}
                    onClick={() => handleSingleSelect('bedrooms', num)}
                    className={`py-2 border rounded-lg text-sm font-poppin transition-all ${
                      filters.bedrooms === num 
                        ? 'bg-[#2B8CAD] text-white border-[#2B8CAD]' 
                        : 'border-gray-200 hover:border-[#2B8CAD] hover:text-[#2B8CAD]'
                    }`}
              ...
              <button 
                onClick={() => handleSingleSelect('bedrooms', 5)}
                className={`w-full py-2 border rounded-lg text-sm font-poppin transition-all ${
                  filters.bedrooms === 5 
                    ? 'bg-[#2B8CAD] text-white border-[#2B8CAD]' 
                    : 'border-gray-200 hover:border-[#2B8CAD] hover:text-[#2B8CAD]'
                }`}
            ...
            {/* Property Type */}
            <div className="mb-6">
              <label className="text-sm font-poppin font-bold text-[#2B8CAD] mb-3 block">Property Type</label>
              <div className="space-y-3">
                {['Single Family', 'Townhouse', 'Condo', 'Multi-Family', 'Apartment', 'House'].map((type) => {
                  const isSelected = filters.propertyTypes.includes(type);
                  return (
                    <label 
                      key={type} 
            ...
            {/* Home Features */}
            <div className="mb-8">
              <label className="text-sm font-poppin font-bold text-[#2B8CAD] mb-3 block">Home Features</label>
              <div className="space-y-3">
                {['Pool', 'Garage', 'Fireplace', 'Hardwood Floors', 'Central AC', 'Dishwasher'].map((feature) => {
                   const isSelected = filters.features.includes(feature);
                   return (
            ...
            {/* Actions */}
            <div className="space-y-3">
              <button className="w-full bg-[#2B8CAD] font-poppin text-white py-3 rounded-lg font-semibold hover:bg-[#237896] transition-colors shadow-sm">
                Apply Filters
              </button>
              <button 
                onClick={resetFilters}
                className="w-full bg-white border border-gray-200 text-gray-700 py-3 rounded-lg font-poppin font-semibold hover:bg-gray-50 transition-colors"
              >
                Reset All
              </button>
            </div>
        ...
        <div className="w-full lg:w-3/4">
          ...
          {filteredProperties.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 bg-white rounded-2xl border border-dashed border-gray-300">
              ...
              <h3 className="text-lg font-poppin font-semibold text-gray-700">No properties found</h3>
              <p className="text-gray-500 text-sm">Try adjusting your filters.</p>
              <button onClick={resetFilters} className="mt-4 text-[#2B8CAD] font-poppin font-medium hover:underline">Clear all filters</button>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredProperties.map((property) => (
                <div key={property.id} className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-300">
                  ...
                  <div className="relative h-46 group">
                    ...
                    {/* Badges */}
                    <div className="absolute top-4 left-4 flex gap-2">
                      <span className="bg-white/95 px-3 py-1 text-[10px] font-poppin font-bold text-gray-800 rounded shadow-sm uppercase tracking-wide">Featured</span>
                      <span className="bg-white/95 px-3 py-1 text-[10px] font-poppin font-bold text-gray-800 rounded shadow-sm uppercase tracking-wide">For Sale</span>
                    </div>
                  ...
                  {/* Content Section */}
                  <div className="p-5 bg-[#f9fcff]">
                    <h3 className="text-base font-poppin font-bold text-gray-800 mb-1">{property.title}</h3>
                    <p className="text-xs font-poppin text-gray-500 mb-2">{property.address}</p>

                    {/* Stats Row */}
                    <div className="flex items-center gap-3 mb-5">
                      <div className="flex items-center gap-1.5 bg-gray-50 px-2 py-1.5 rounded border border-gray-100 min-w-[60px] justify-center">
                        <Bed size={14} className="text-gray-400" />
                        <span className="text-xs font-poppin font-medium text-gray-600">x{property.beds}</span>
                      </div>
                      <div className="flex items-center gap-1.5 bg-gray-50 px-2 py-1.5 rounded border border-gray-100 min-w-[60px] justify-center">
                        <Bath size={14} className="text-gray-400" />
                        <span className="text-xs font-poppin font-medium text-gray-600">x{property.baths}</span>
                      </div>
                      <div className="flex items-center gap-1.5 bg-gray-50 px-2 py-1.5 rounded border border-gray-100 min-w-[80px] justify-center">
                        <Maximize size={14} className="text-gray-400" />
                    ...
                    {/* Footer */}
                    <div className="flex justify-between items-center pt-2 border-t border-gray-50">
                      <button className="bg-[#2B8CAD] hover:bg-[#237896] text-white text-sm font-poppin font-semibold px-5 py-2 rounded-lg transition-colors shadow-sm">
                        {property.price}
                      </button>
                      <button className="w-9 h-9 flex items-center justify-center bg-gray-100 rounded-full hover:bg-red-50 hover:text-red-500 transition-colors text-gray-400">
                        <Heart size={16} />
          ...
          {filteredProperties.length > 0 && (
            <div className="mt-8 flex justify-center">
               <button className="text-[#2B8CAD] text-sm font-poppin font-semibold hover:underline">Load More Listings</button>
            </div>
          )}
        </div>
